...
void pwm_init()
{
    ...
    ledc_channel_config_t ul = {};

    ul.gpio_num = UL_GPIO;
    ul.speed_mode = LEDC_LOW_SPEED_MODE;
    ul.channel = LEDC_CHANNEL_1;
    ul.intr_type = LEDC_INTR_DISABLE;
    ul.timer_sel = LEDC_TIMER_0;
    ul.duty = 0;
    ul.hpoint = 0;

    ledc_channel_config(&ul);
    ...
    ledc_channel_config_t vl = {};

    vl.gpio_num = VL_GPIO;
    vl.speed_mode = LEDC_LOW_SPEED_MODE;
    vl.channel = LEDC_CHANNEL_3;
    vl.intr_type = LEDC_INTR_DISABLE;
    vl.timer_sel = LEDC_TIMER_0;
    vl.duty = 0;
    vl.hpoint = 0;

    ledc_channel_config(&vl);
    ...
    ledc_channel_config_t wl = {};

    wl.gpio_num = WL_GPIO;
    wl.speed_mode = LEDC_LOW_SPEED_MODE;
    wl.channel = LEDC_CHANNEL_5;
    wl.intr_type = LEDC_INTR_DISABLE;
    wl.timer_sel = LEDC_TIMER_0;
    wl.duty = 0;
    wl.hpoint = 0;

    ledc_channel_config(&wl);
...
)
{
    ...
    {
        case 0: // UH + VL
            set_pwm(duty,0,0,255,0,0);
            break;

        case 1: // UH + WL
            set_pwm(duty,0,0,0,0,255);
            break;

        case 2: // VH + WL
            set_pwm(0,0,duty,0,0,255);
            break;

        case 3: // VH + UL
            set_pwm(0,255,duty,0,0,0);
            break;

        case 4: // WH + UL
            set_pwm(0,255,0,0,duty,0);
            break;

        case 5: // WH + VL
            set_pwm(0,0,0,255,duty,0);
            break;
    }
}
    ...
    while(1)
    {
        ...
        if(rx_len > 0)
        {
            ...
            if(tm.valid_telemetry)
            {
                ...
                const int DUTY_BASE = 80;

                int duty = 
                    DUTY_BASE +
                    (int)(fabs(alpha_rw) * 2.0f);
